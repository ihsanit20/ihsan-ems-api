...
class StudentController extends Controller
{
     ...
     * - per_page (default: 25)
     * - with_latest_enrollment (boolean, default: true - include latest enrollment with minimal related data)
    ...
    public function index(Request $request)
    {
        ...
        // ---------------- Eager loading (explicit "with" param) ----------------
        ...
        if ($with) {
            // NOTE: explicit with দিলে আগের মতই লোড হবে (full),
            // কারণ এটা advanced/details পেজে লাগতে পারে।
            $query->with($with);
        }

        // ---------------- Latest enrollment (default slim) ----------------
        if ($request->boolean('with_latest_enrollment', true)) {
            $query->with(['enrollments' => function ($q) {
                $q->select([
                    ...
                    ->limit(1)
                    ->with([
                        // ✅ minimal columns only (id + name + required FK)
     ...
     * - with_latest_enrollment (boolean, default: true - include latest enrollment with minimal details)
     * - enrollment_slim (boolean, default: false - if true, slim all enrollments too)
     */
    public function show(Student $student, Request $request)
    {
        $with = ['user'];

        $enrollmentSlim = $request->boolean('enrollment_slim', false);

        if ($request->boolean('with_enrollments', true)) {
            if ($enrollmentSlim) {
                // ✅ slim all enrollments
                $with[] = 'enrollments:id,student_id,academic_session_id,session_grade_id,section_id,roll_no,status,admission_type';
                $with[] = 'enrollments.academicSession:id,name,is_active';
                $with[] = 'enrollments.sessionGrade:id,academic_session_id,grade_id';
                $with[] = 'enrollments.sessionGrade.grade:id,level_id,name';
                $with[] = 'enrollments.sessionGrade.grade.level:id,name';
                $with[] = 'enrollments.section:id,session_grade_id,name';
            } else {
                // previous full details
        ...
        if ($request->boolean('with_latest_enrollment', true)) {
            // ✅ slim latest enrollment for show too
            $student->latest_enrollment = $student->getLatestEnrollmentWithDetailsSlim();
        }
    ...
    {
        // keep full history for dedicated endpoint
    ...
    public function export(Request $request)
    {
        $students = Student::query()
            ...
            ->with([
                'enrollments:id,student_id,academic_session_id,session_grade_id,section_id,roll_no,status',
                'enrollments.academicSession:id,name',
                'enrollments.sessionGrade:id,academic_session_id,grade_id',
                'enrollments.sessionGrade.grade:id,name',
                'enrollments.section:id,name'
            ])
            ->get()
